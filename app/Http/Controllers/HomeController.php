...
class HomeController extends Controller
{
     ...
     *
     * **What is stored is rows, never models.** `config('cache.serializable_classes')`
     * is `false` here, which tells Laravel to unserialize no PHP class at all
     * from the cache: if the APP_KEY ever leaks, a writable cache must not become
     * a gadget chain. A cached Eloquent model comes back as
     * `__PHP_Incomplete_Class` under that setting, so the cache holds plain
     * attribute arrays and `hydrate()` rebuilds the models on the way out. Casts
     * are reapplied there, which is what keeps `published_at` a date the view can
     * format rather than the string the database returned.
     *
    ...
    private static function cards(string $type, int $limit): Collection
    {
        ...
        $rows = Cache::remember($key, 300, static fn (): array => Post::published()
